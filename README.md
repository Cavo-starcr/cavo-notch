...
Собирает `build/Cyclop-<версия>.dmg` — приложение и ярлык `/Applications` рядом,
установка одним перетаскиванием.

Образ не нотаризован: подпись ad-hoc, Developer ID нет. На своей машине это
ничему не мешает, но у того, кому образ отдали, Gatekeeper приложение не пустит,
пока он один раз не разрешит его в Системных настройках → Конфиденциальность и
безопасность → «Все равно открыть». В macOS 15 открытие через Control-клик для
такого случая больше не работает. Убирается это только платным Developer ID и
нотаризацией.
